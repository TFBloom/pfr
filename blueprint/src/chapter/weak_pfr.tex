...
and there exists an integer $k\geq 0$ with
\[\log \lvert H\rvert \leq \left(2-2^{2-k}\right)(\mathbb{H}(X)+\mathbb{H}(Y))\]
and
\[\mathbb{H}(\psi(X))+\mathbb{H}(\psi(Y))\leq 2^{1-k}(\mathbb{H}(X)+\mathbb{H}(Y)).\]
Note that this exists since $H=\{0\}$ is an example of such a subgroup (with $k=1$) or we are done with this choice of $H$.
...
\[\log \lvert H''\rvert=\log \lvert H'\rvert+\log \lvert H\rvert< (2-2^{1-k})(\mathbb{H}(X)+\mathbb{H}(Y)).\]
and
\[\mathbb{H}(\psi''(X))+\mathbb{H}(\psi''(Y))< \frac{\mathbb{H}(\psi(X))+\mathbb{H}(\psi(Y))}{2}\leq 2^{k}(\mathbb{H}(X)+\mathbb{H}(Y)).\]
Therefore $H''$ satisfies the second and third conditions (with $k$ replaced by $k+1$) and so (using the maximality of $H$) it must be the first condition that fails, whence
